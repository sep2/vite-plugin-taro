...
原生 Socket 打开后，运行时只发送 `{ kind: 'startup', buildId }`，表示一个新的 App 运行环境从磁盘基线启动。主机如果尚未发布任何补丁，就继续运行；只要已经发布过补丁，就从当前源码完整构建，刷新基线和构建身份。这个判断只看本次完整构建后是否发布过补丁，与补丁是否已确认、历史是否仍完整无关，不尝试启动重放。新 App 遇到缺少序号 1 的后缀时先等待这条恢复路径，不安装不完整的模块工厂。
