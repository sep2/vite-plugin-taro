...
| 动态边界后的静态依赖可能跨越主包和多个分包 | 共享模块运行时按原依赖图连接这些模块；引用方和依赖不必位于同一分包，共享依赖也无需复制 |
...
应用最终只表达**同步依赖**与**异步加载边界**，不表达微信包路径，使业务代码与分包关系完全解耦。分包只是模块的物理存放位置，不是源码中的模块边界。
...
静态 `import` 只表达同步执行关系，不要求引用方和依赖位于同一个物理包。从 App、页面或其他启动代码同步可达的模块属于**同步启动图**，因此都留在主包；进入动态导入边界后，每个模块的位置会独立规划。

所以，一个分包中的模块可以静态引用：

- 已经被同步启动图使用、因而位于主包的共享模块；
- 位于同一个分包的模块；
- 位于另一个分包的模块。
...
动态 `import()` 创建加载边界，但一个动态边界不等于一个分包。边界后的模块会根据是否也被同步启动图使用、转换后体积、分包剩余空间、共同动态入口和直接静态依赖关系分别确定位置。静态导入关系只是尽量同包的偏好，不是强制约束。
