...
微信、支付宝和抖音监听构建会自动关闭生成项目配置中的开发者工具热重载，避免它干扰完整重建；微信和抖音的私有热重载设置也会被覆盖。抖音的自动编译设置保持原值。切回 `dev:...` 后使用 `vite.config.ts` 中原有的配置，无需手动切换。具体字段见[项目配置](/guides/configuration/#projectconfigjson)。
