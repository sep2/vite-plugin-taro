---
title: 全自动分包
description: 使用动态导入，让 vpt 自动管理微信、支付宝与抖音小程序的主包和分包。
---

import AutomaticSubpackagesDiagram from '../../../components/automatic-subpackages-diagram.astro'
import FloatingContent from '../../../components/floating-content.astro'
...
:::note
不需要手动配置任何 `subPackages`。分包是全自动的。
:::

<FloatingContent
    id="automatic-subpackages-diagram"
    label="自动分包规划示意图"
    description="查看源码依赖如何自动规划为主包与分包"
    collapseLabel="收起自动分包规划示意图"
    collapseText="收起示意图"
    expandLabel="展开自动分包规划示意图"
    inlineSize="76rem"
    blockSize="46.6rem"
>
    <AutomaticSubpackagesDiagram headingLevel={2} />
</FloatingContent>
...
当然，页面内容也可以使用原生组件；按照[原生组件](/guides/native-components/)设置后，可以通过同样的 `<Suspense>` 按需加载。
...
小程序原生组件会参与位置规划。vpt 会把原生组件目录的文件体积计入声明模块，并将目录输出到该模块最终所属的主包或分包。接入方式、类型 schema 和动态加载示例参见[原生组件](/guides/native-components/)。
...
位置规划算法、跨分包加载方式和小程序运行时原理参见[模块系统](/references/module-system/)。
