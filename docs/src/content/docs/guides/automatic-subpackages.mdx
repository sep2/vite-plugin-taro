...
不要在 `appJson` 中手动配置 `subPackages` 或 `subpackages`。位置规划算法、跨分包加载方式和微信运行时原理参见[模块系统](/vite-plugin-taro/references/module-system/)。

## 原生组件与其他资源

通过 `defineNativeComponent()` 声明的[微信原生组件](/vite-plugin-taro/guides/native-components/)会参与位置规划。vpt 会把原生组件目录的文件体积计入声明模块，并将目录输出到该模块最终所属的主包或分包。
