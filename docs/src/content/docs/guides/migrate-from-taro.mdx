...
:::note[适用范围]
vpt 当前构建 **React 19 + 微信小程序 + Web**。Vue、Preact、Solid、React Native 以及其他小程序目标不能直接迁移。本指南对照了 [Taro 4.2.1 官方项目模板](https://github.com/NervJS/taro/tree/v4.2.1/packages/taro-cli/templates/default)及 vpt 的实际构建实现。
:::
...
```sh
yarn create vite-taro my-app-vpt
cd my-app-vpt
yarn install
```
...
`Taro.request()`、`Taro.navigateTo()`、`useLaunch()`、`useLoad()` 等调用方式不变。`process.env.TARO_ENV` 也可以继续用于目标判断：微信、支付宝、抖音和 Web 构建中的值分别为 `weapp`、`alipay`、`tt` 和 `h5`。
...
上面以 WX/H5 配置为例。微信、支付宝与抖音构建都应使用对应平台的原生字段，vpt 不执行 Taro CLI 的配置转换。例如，ZFB 分支应把 `navigationBarTitleText` 写为 `defaultTitle`、把 `enablePullDownRefresh` 写为 `pullRefresh`，并使用支付宝的 tabBar 结构；TT 分支应按抖音文档提供应用和页面配置，不要复制微信专属的 Skyline 字段。可以参考生成模板中的 `createAppJson(target)` 和 `createPageJson(target)`。
...
`projectConfigJson` 和 `sitemapJson` 在类型上始终需要提供，但只会在小程序构建中写出。
...
`designWidth`、`deviceRatio` 和 Taro `pxtransform` 构建配置没有可直接复制到 `vpt()` 的选项。微信、支付宝和抖音的应用 CSS 默认按 750px 设计稿转换 `px` / `rem`；运行时 `Taro.pxTransform()` 在小程序也可用，但单独调用 `Taro.initPxTransform()` 不会更改静态 CSS。需要其他设计宽度时，按[样式指南的尺寸配置](/guides/styles/#使用其他设计稿宽度)分别设置 Vite PostCSS 和运行时，再逐页检查尺寸。H5 不会自动将 CSS 的 `px` 转成 `rem`，也不会自动初始化 `pxTransform`。
...
构建时仍会替换 `process.env.TARO_ENV`：wx、zfb、tt 和 h5 构建中的值分别为 `weapp`、`alipay`、`tt` 和 `h5`。如果迁移后的 TypeScript 配置不再声明 `process`，优先改用下面的注释指令；也可以在项目自己的类型文件中为 `process.env.TARO_ENV` 添加窄类型声明。
