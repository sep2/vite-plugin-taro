---
title: 从 Taro CLI 迁移
description: 将现有 Taro React 项目迁移到 vite-plugin-taro。
---
...
请先完整阅读 vpt 的文档，了解配置、页面约定、全自动分包和当前支持范围，再开始迁移。
...
请先把页面整理为普通主包页面，再使用动态 `import()` 延迟加载页面内部的大型功能。具体行为参见[全自动分包](/vite-plugin-taro/guides/automatic-subpackages/)。
...
:::caution[第三方 Taro 库]
依赖 `@tarojs/*` peer dependency、Taro CLI 插件、webpack loader 或 React 18 内部行为的第三方库不一定兼容。先确认库支持 React 19，再在微信和 Web 两端分别验证。无法兼容的构建插件需要替换，而不是继续保留旧 Taro CLI。
:::
