...
`Taro.request()`、`Taro.navigateTo()`、`useLaunch()`、`useLoad()` 等调用方式不变。`process.env.TARO_ENV` 也可以继续用于目标判断：微信、支付宝和 Web 构建中的值分别为 `weapp`、`alipay` 和 `h5`。
...
上面的字段适用于 WX/H5。ZFB 构建不会执行 Taro CLI 的配置转换，因此应在 `vite.config.ts` 的 ZFB 分支直接提供支付宝字段，例如把 `navigationBarTitleText` 写为 `defaultTitle`、把 `enablePullDownRefresh` 写为 `pullRefresh`，并使用支付宝的 tabBar 结构。可以参考生成模板中的 `createAppJson(target)` 和 `createPageJson(target)`。
...
构建时仍会替换 `process.env.TARO_ENV`：WX、ZFB 和 H5 构建中的值分别为 `weapp`、`alipay` 和 `h5`。如果迁移后的 TypeScript 配置不再声明 `process`，优先改用下面的注释指令；也可以在项目自己的类型文件中为 `process.env.TARO_ENV` 添加窄类型声明。
