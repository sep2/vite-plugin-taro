...
A four-target VPT example with real Home, Explore, and About tab routes and one React tab bar shared by WeChat, Alipay, TikTok, and H5. Home keeps the default starter page intact. Tab metadata lives in `src/tab-pages.ts`; icons are imported from `src/assets/icons`.

WeChat and Alipay require an empty native custom-tab-bar slot so their default bars never flash before the React App renders. `vite.config.ts` copies only the current target's files from `native-tab-bar-stubs/`. TikTok uses `tabBar.custom`. Navigation uses `Taro.switchTab`, not `reLaunch`.
