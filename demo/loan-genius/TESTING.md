...
首次授权时按开发者工具提示确认。测试默认使用 `Pi` 客户端；如果已授权的名称不同，请在运行测试前设置环境变量 `VPT_LOAN_HMR_DEVTOOLS_CLIENT`。

### 覆盖范围

测试包含 27 个有状态流程，检查计算器状态在以下场景中保留或按预期重置：
...
示例配置了 `polyfills: ['web.url']`，不配置 `URL` 的 `define` 映射。对应流程先验证启动时的 `globalThis.URL`，通过 HMR 切换为裸 `URL`，再还原代码，同时检查计算器状态。这样验证的是两种访问方式本身，而不是把裸 `URL` 改写为属性访问。
...
| 目标 | 当前覆盖范围 |
| --- | --- |
| 微信（WX） | 构建检查与 27 流程开发者工具 HMR 套件；不覆盖 Skyline。 |
