...
const currentVersion = packageJsonFiles[0].json.version
...
const templateDevDependencies =
    templatePackageJson.json.devDependencies ?? fail(`${templatePackageJsonPath} is missing devDependencies.`)
...
function resolveVersion(value, currentVersion, prereleaseIdentifier) {
...
function initialPrerelease(prereleaseIdentifier) {
    return prereleaseIdentifier ? [...validatePreid(prereleaseIdentifier), '0'] : ['0']
}

function incrementPrerelease(prerelease, prereleaseIdentifier) {
...
function findLastNumericIdentifierIndex(identifiers) {
...
function updateVersionSpecifier(specifier, nextVersion) {
