...
check(document === runtimeDocument, 'injected document identity')
check(typeof window === 'undefined', 'window must not be injected')
check(runtimeWindow.document === document, 'explicit Taro window.document identity')
...
check(typeof runtimeWindow.getComputedStyle === 'function', 'explicit Taro window computed style API')
