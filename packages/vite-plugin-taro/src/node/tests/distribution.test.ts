...
test('uses source in the workspace and publishes a size-bounded compiler plus client modules', async () => {
    ...
    assert.doesNotMatch(compiler, /from\s*['"]@tarojs\/plugin-platform-(?:alipay|weapp)['"]/)
    assert.match(compiler, /@tarojs\/plugin-platform-weapp\/dist\/runtime-utils\.js/)
    assert.match(compiler, /@tarojs\/plugin-platform-alipay\/dist\/runtime-utils\.js/)
