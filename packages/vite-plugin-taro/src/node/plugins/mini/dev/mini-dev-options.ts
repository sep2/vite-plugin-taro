...
/**
 * Applies physical Mini Program output and runtime conventions through Rolldown's public options hook.
 * Build completion deliberately remains outside this options adapter: DevEngine onOutput is the single lifecycle authority.
 */
export function createMiniDevOptionsPlugin({
    server,
    contract,
    hmrMode
}: {
    ...
    return {
        name: 'vpt:mini-dev-options',
        /*
         * Each DevEngine applies this hook to its own Vite-resolved options before binding the native graph. Keeping adaptation
         * in Rolldown's lifecycle leaves Vite's options factory intact and avoids a second engine with duplicate watchers or graphs.
         */
        async options(rolldownOptions: RolldownOptions): Promise<RolldownOptions> {
            ...
            /*
             * DevEngine receives this mutable output object separately before options hooks run, and nested Vite plugins may retain it.
             * Mutating that single object preserves their references while applying the shared output configuration and native runtime format.
...
/*
 * memoize owns one mutable Promise per runtime entry. Options hooks run for each engine, and separate Vite servers may select
 * different targets or modes in one process. Sharing each in-flight/result Promise prevents duplicate
