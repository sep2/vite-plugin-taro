...
test('projects Rolldown physical chunk paths into package-neutral SystemJS IDs', () => {
    assert.equal(toLogicalChunkId('app-capsule.js'), 'app-capsule.js')
    assert.equal(toLogicalChunkId('pages/home-capsule.js'), 'pages/home-capsule.js')
})
...
test('keeps physical reference resolution separate from logical identity projection', () => {
    ...
    assert.equal(resolveLogicalChunkReference('assets/pages/home.js', '../shared.js'), 'assets/shared.js')
})
