...
export const equalInterestCalc = ({
    ...
    new Promise((resolve) => {
        ...
        const commercePayMonty: number = getInterestEveryMonth(commerceTotalPirce, commerceLoanRate / 12, commerceMonth)
 ...
 * @return
 */
const getInterestEveryMonth = (loanTotalPrice: number, monthLoanRate: number, monthNum: number): number => {
    if ((1 + monthLoanRate) ** monthNum - 1 === 0) {
        return 0
    }
    return (loanTotalPrice * monthLoanRate * (1 + monthLoanRate) ** monthNum) / ((1 + monthLoanRate) ** monthNum - 1)
}
 ...
 * @return
 */
const getInterestTotal = (repaymentTotal: number, loanTotalPrice: number): number => {
...
const getPrincipalListByMonth = (loanTotalPrice: number, monthLoanRate: number, monthNum: number): number[] => {
    const accumulatList: number[] = []
    let finshRepayTotal = 0
    for (let i = 0; i < monthNum; i++) {
        const repayMonth = loanTotalPrice / monthNum + (loanTotalPrice - finshRepayTotal) * monthLoanRate
 ...
 * @return
 */
const mergeList = (commList: number[], accuList: number[], commMonthNum: number, accuMonthNum: number): number[] => {
