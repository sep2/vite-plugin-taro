...
这为 HMR 留出了空间。页面代码可以重新执行，而 HMR runtime、模块注册表、Taro root、React families 和 Fiber tree 会因为 App 代码没有重新执行而继续存在。

插件为此保留了一个页面直接依赖：**更新桥（update bridge）**，输出文件名为 `update.js`。开发环境首次启动时它是空的。每次出现兼容的 JavaScript 修改，开发服务器都会把生成的 patch 代码放进同一个更新桥。对 DevTools 来说，变化的始终只是这个已知的页面依赖。

DevTools 重新执行页面时，会在页面常规初始化之前进入更新桥。更新桥调用已经存在的 HMR runtime，由它应用 patch。之后其余页面代码可以继续执行，但不再负责持有应用状态。
...
sequenceDiagram
    ...
    alt 兼容的 JavaScript 更新
        Server->>WX: 把 patch 代码写入更新桥
        WX->>WX: 重新执行页面代码并保留 App 代码
        WX->>WX: 更新桥调用 HMR runtime
...
- **执行通道**就是更新桥。开发服务器把 JavaScript 写入项目，DevTools 再通过小程序的正常工具链编译它。

控制通道决定缺少哪些 patch；执行通道负责真正运行这些 patch。

## 初始开发构建

开发模式使用一个 eager 的 Vite/Rolldown 模块图。同一个模块图既生成初始完整原生 bundle，也就是 **baseline**，也生成后续所有 patch。这样 baseline 与更新之间的模块 ID、transform、依赖边界和 React Refresh instrumentation 都保持一致。
...
5. 客户端通过控制通道报告当前版本。
6. 服务器把客户端缺少的连续 patch 范围写入更新桥。
7. DevTools 发现页面依赖变化，只重新执行当前页面代码，不重新执行 App。
8. 更新桥调用 HMR runtime，由它按顺序应用 patch。
9. React Refresh 使用保留下来的 Fiber tree 协调新的组件实现。
10. 只有 Refresh 完成后，客户端才确认新版本。

源码修改可能来自 App 组件、当前页面、未打开的页面或共享依赖。在兼容路径中，开发服务器不会把该源码文件作为单独的原生文件变化暴露给 DevTools。DevTools 看到的可执行文件变化只有更新桥。
...
Runtime 还会记录由 patch 初始化的模块。DevTools 继续执行页面原有 bundle 时，旧的 baseline initializer 不能覆盖刚刚更新的模块。遇到这种情况，initializer 会返回当前 patch 后的 exports。

因此执行顺序是安全的：

- 更新桥先安装新的模块实现；
- 剩余页面代码继续执行；
- 旧 baseline 的初始化不能覆盖新实现。

### 保留 React 状态

HMR runtime 使用 Vite 官方的 React Refresh runtime。Rolldown 更新模块 exports 并触发 HMR 边界，React Refresh 判断受影响的组件 families 是否兼容。

对于兼容的 families，React 会复用现有 Fiber tree 和组件状态。Fiber tree 不会被序列化进更新桥，也不会复制到 patch 中。它只是因为 App 代码和 Taro root 没有被销毁而继续可达。

如果 React 报告无法安全刷新的 stale family，当前路由会重新启动。状态保留只适用于 React Refresh 认为兼容的更新。

### 让页面重新执行不破坏状态

页面入口属于原生集成代码，可能再次执行。HMR runtime 把它视为可重复的初始化，而不是长期状态的所有者。
...
- 当前完整 baseline 的 **build ID**；
- 单调递增的 **patch version**；
- 该 baseline 下保留的 patch；
- 当前 HMR 客户端 **session**；
- 最多一个等待确认的已发布 patch 范围。

客户端持续报告已经实际应用的版本。如果客户端落后，服务器会把一段连续缺失的 patch 写入更新桥。该范围确认之前到达的新修改，会等待下一次发布。
...
开发服务器仍在运行时，DevTools 可能会重载 App。新的 HMR 客户端会创建新 session，并从版本 0 开始。服务器随后重新发布当前 baseline 以来保留的所有 patch。

提前初始化页面可以确保这些 patch 被正确重放，包括重载后尚未打开的路由。

### 开发服务器重启

Patch 历史只保存在服务器内存中。Vite 重启后会创建新的完整 baseline 和 build ID。最新源码已经包含在这个 bundle 中，因此旧 patch 历史既不需要，也不应继续信任。

新的 baseline 会把更新桥和 patch version 重置为 0。

### 有界历史

保留的 patch 历史有容量上限。历史过大时，服务器会创建新的完整 baseline，并清除旧历史，避免长时间开发后积累无限增长的重放日志。
...
因此，固定更新桥不是任意选择的传输方式，而是同时满足“执行新代码”和“保持当前 App 不变”的最小机制。

## 为什么这个设计是充分的

对于兼容更新，所需条件都会成立：

1. DevTools 从更新桥编译 patch 代码。
2. 只有页面代码重新执行，App 代码和已有 JavaScript 状态保持不变。
3. HMR runtime 把 patch 应用到现有模块注册表。
4. 页面重新执行保护机制阻止 baseline 代码撤销 patch。
...
如果页面已经开始重新执行后才发现依赖，就无法及时建立安全重载边界。更新桥必须在初始 bundle 中就作为页面的直接静态依赖存在。
...
- Taro root；
- 原生输入状态；
- 兼容组件 families 的 React 状态。

它不保证保留：

- 任意模块局部 singleton 状态；
- 不兼容的 React 组件 families；
- 完整原生构建前后的状态；
- 开发服务器重启前后的状态。
