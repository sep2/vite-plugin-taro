...
文件是空的。每次出现兼容的 JavaScript 修改，开发服务器都会把生成的 patch 代码放进这个文件。对
...
对于 React Refresh 判断为兼容的组件更新，React 会复用现有 Fiber tree 和组件状态。Fiber tree 不会被
序列化进 `update.js`，也不会复制到 patch 中。它只是因为已有 App 实例和 Taro root 没有被销毁而继续可达。
...
只有能够表示为安全 JavaScript patch 的更新才走当前的状态保留路径。如果修改涉及或可能涉及以下内容，
就必须进行完整原生构建：
...
`app.wxss`，无法让普通源码 CSS 使用这条边界。在实现 CSS ownership 和路由拆分之前，CSS 修改仍然走
完整构建路径。保留状态的 WXSS 热更新是下一项计划扩展，即将支持。
