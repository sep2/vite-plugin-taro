...
保留状态的热更新要求微信项目设置为 `compileHotReLoad: true`。下文描述的行为都以该设置保持启用为前提。

裸微信项目的 DevTools 探针表明，`page.js` 以及由初始页面代码直接加载的文件具有可用于 JavaScript
热更新的重载边界。

这里的“直接 `require()`”有明确含义：`page.js` 从首次构建开始就包含类似
`require('./update.js')` 的字面量调用。通过另一个模块间接加载，或者在运行时拼接路径再加载，都不属于
这个边界。

当 `page.js` 本身或这样直接加载的文件发生变化时：

- DevTools 会重新编译并执行页面侧代码；
- `app.js` 不会重新执行；
- App 实例、`globalData` 和 App 持有的 runtime 仍然可用。

这为 JavaScript HMR 留出了空间。页面代码会重新执行，但 `app.js` 不会重新执行；已有的 App 实例、HMR
runtime、模块注册表、Taro root 和 Fiber tree 会继续存在。

插件据此让每个 `page.js` 从首次构建开始就直接 `require()` 同一个 `update.js`。开发环境首次启动时这个
文件是空的。每次由 HMR 处理 JavaScript 修改时，开发服务器都会把生成的 patch 代码放进这个文件。对
DevTools 来说，变化的始终只是这个页面已经直接加载过的文件。

DevTools 重新执行页面时，会在页面常规初始化之前执行 `update.js`。该文件调用已经存在的 HMR runtime，
由它应用 patch。之后页面入口的其余代码继续执行；HMR runtime 会继续使用 patch 后的模块 exports，并确保
页面只注册一次。

核心设计可以概括为：

> 把 HMR 处理的源码修改转换为一个页面级可执行文件的变化，让微信只重新执行页面，同时保留 App 和
> React 状态。

每次 HMR 更新都**只会写入 `update.js` 这一个项目文件**。更新后的模块代码和 patch 历史保留在
开发服务器内存中；`app.js`、页面 bundle、共享 chunk 和原生文件都不会被改写。服务器只把当前缺少的
patch 范围写入 `update.js`。因此 DevTools 只能观察到这个已知页面依赖发生变化，不会因为其他文件变化
而触发 App 重载。
...
2. 每个 `page.js` 都通过字面量 `require()` 直接加载空的 `update.js` 文件。
3. 所有已配置的页面组件都提前初始化。
...
无论修改来自 App 组件、当前页面、未打开的页面还是共享依赖，都使用相同的更新流程。

## HMR runtime

由于 `app.js` 没有重新执行，挂在已有 App runtime 上的 HMR runtime 可以在页面代码重新执行后继续存在。它有三个主要职责。
...
HMR runtime 使用 Vite 官方的 React Refresh runtime。Rolldown 更新模块 exports 并触发 HMR 边界，React Refresh 判断受影响的组件状态能否安全复用。

React Refresh 会判断受影响的组件状态能否复用。能够复用时，React 会继续使用现有 Fiber tree 和组件
状态。Fiber tree 不会被序列化进 `update.js`，也不会复制到 patch 中；它只是因为已有 App 实例和 Taro
root 没有被销毁而继续可达。

如果 React Refresh 无法安全更新某个组件，当前路由会重新启动，该组件状态也会重置。

### 让页面重新执行不破坏状态

页面入口属于原生集成代码，并会在这类更新中再次执行。HMR runtime 把它视为重复的集成初始化，并处理其中的重复执行副作用。
...
状态保留 HMR 只接受安全的 JavaScript patch。如果修改涉及或可能涉及以下内容，就必须进行完整原生构建：

- 当前输出到 `app.wxss` 的应用 CSS，包括新增的 Tailwind utilities；
- WXML、JSON、项目配置或页面配置；
- 导入资源或 public 文件；
...
DevTools 可以安全地直接更新页面持有的 `page.wxss`，且不会替换 App 状态。当前插件把样式汇总到
`app.wxss`，无法让普通源码 CSS 使用 `page.wxss` 边界。在实现 CSS ownership 和路由拆分之前，CSS 修改仍需
完整构建。保留状态的 WXSS 热更新是下一项计划扩展，即将支持。

在已测试的 DevTools 环境中，改写 `app.wxss` 的构建会重载 App，因此旧 JavaScript 堆、HMR runtime、
Taro root 和 Fiber tree 都会消失，之后的 HMR 逻辑无法恢复它们。
...
一次 JavaScript HMR 更新会满足全部必要条件：

1. DevTools 从 `update.js` 编译 patch 代码。
2. 只有页面代码重新执行；`app.js` 不会重新执行，已有 App 实例和 JavaScript 状态保持不变。
...
安全边界取决于具体文件，并不意味着可以任意重写生成产物。页面 JavaScript 和 `page.wxss` 可以避免 App
重载，但替换原始 JavaScript bundle 会绕过活动 patch registry 与 React Refresh 协调，而替换
`app.wxss` 已被证明会重载 App。固定的 `update.js` 依赖同时隔离了这两类风险。
...
- Taro root；
- 原生输入状态；
- React Refresh 能够保留的组件状态。

它不保证保留：

- 任意模块局部 singleton 状态；
- React Refresh 无法保留的组件状态；
- 完整原生构建前后的状态；
- 开发服务器重启前后的状态。

在 DevTools 层面，直接保存 `page.wxss` 也会保留 App 和页面侧 identity，而保存 `app.wxss` 会替换它们。
