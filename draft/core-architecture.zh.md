...
Vite 收集到的应用 CSS 会输出为原生 WXSS。Taro 原生组件和模板配置再让这些样式应用到渲染后的页面 tree。

WX HMR 期间，JavaScript patch 可以继续使用当前 WXSS 中已经存在的 utility classes。如果引入新的 Tailwind candidate，就必须执行完整原生构建，避免 JavaScript class names 与 WXSS 不一致。详细更新协议见 `draft/hmr-architecture.md`。
...
H5 使用 Vite 的普通浏览器开发模型。浏览器从 Vite server 加载模块，React Fast Refresh 应用兼容的组件更新。

### WX 开发

WX 开发使用 Vite bundled Rolldown graph，因为 DevTools 消费的是原生项目目录，而不是浏览器模块。兼容的 JavaScript 更新保留在内存中，并通过 `update.js` 交付；这类更新只会改写这一个项目文件。CSS、原生配置、资源和不安全的修改会触发完整原生构建。
